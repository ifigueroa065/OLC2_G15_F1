class UDiv extends Instruction {
    ...
    execute(env, gen) {
        ...
        console.log('Resultado udiv', result);
        if (result === 0) {
            ZF = true;
        } else if (result < 0) {
            NF = true;
        ...
        console.log('FLAGS udiv');
        console.log('Memory Manager Flags', memoryManager.getFlags());

        //pruebas

        // Generar el cuádruplo para la instrucción UDIV
        if (retornoDestino && retornoSource1 && retornoSource2) {
            gen.addQuadruple('UDIV', retornoSource1.valor, retornoSource2.valor, null, null, null, retornoDestino.valor);
        } else {
