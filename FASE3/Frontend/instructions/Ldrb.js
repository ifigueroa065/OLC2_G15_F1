class Ldrb extends Instruction {
    ...
    execute(env, gen) {
        ...
        if (retornoDestino && retornoSources.every(r => r !== null)) {
            if (retornoSources.length === 1) {
                gen.addQuadruple('LDRB', retornoSources[0].valor, null, null, null, null, retornoDestino.valor);
            } else if (retornoSources.length === 2) {
                gen.addQuadruple('LDRB', retornoSources[0].valor, retornoSources[1].valor, null, null, null, retornoDestino.valor);
            }
        } else {
