class Mvn extends Instruction {
    constructor(destination, source, line, col) {
        super();
        this.destination = destination;
        this.source = source;
        this.line = line;
        this.col = col;
    }

    execute(env, gen) {
        // Obtener los valores de los operandos
        let retornoDestino = this.getRetorno(this.destination, env);
        let retornoSource = this.getRetorno(this.source, env);

        // Generar el cuádruplo para la instrucción MVN
        if (retornoDestino && retornoSource) {
            gen.addQuadruple('MVN', retornoSource.valor, null, null, null, null, retornoDestino.valor);
        } else {
            console.error('Error: Uno o más valores de retorno son undefined');
        }
    }

    getRetorno(node, env) {
        if (!node) return null;
        if (node._InsExpr) return node._InsExpr.execute(env);
        if (node.children && node.children.length > 0) {
            let child = node.children[0];
            if (child._InsExpr) return child._InsExpr.execute(env);
            if (child.children && child.children.length > 0) {
                return child.children[0]._InsExpr ? child.children[0]._InsExpr.execute(env) : null;
            }
        }
        return null;
    }
}
